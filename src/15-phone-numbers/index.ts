 ...
 * number. The first three digits of the local number represent the exchange code,
...
class PhoneNumber {
  ...
  constructor(input: string) {
    this.phoneNum = input.replace(/\D+/g, "");
  }

  number() {

    if (this.phoneNum.length > 11 || this.phoneNum.length < 10) return null;
    if (this.phoneNum.length > 10 && this.phoneNum[0] != "1") return null;
    if (this.phoneNum.length > 10) this.phoneNum = this.phoneNum.slice(1, 11);

    return (this.phoneNum[0] == "0"
      || this.phoneNum[0] == "1"
      || this.phoneNum[3] == "0"
      || this.phoneNum[3] == "1")
      ? null : this.phoneNum;
